...
require_once __DIR__ . '/../classes/uploader.config.php';
...
$_TRANSLATIONS["ua"] = array(
	...
	"chmod_dir_failed" => "Не удалось змінити права доступу до теки",
...
class ImageServer
{
	...
	{
		global $_CONFIG;
		if (!file_exists($file)) $file = $_CONFIG['basedir'].$file;
...
class GateKeeper
{
	public static function init()
	{
		...
		if(isset($_POST['user_pass']) && strlen($_POST['user_pass']) > 0)
		{
			...
			{
				$_SESSION['ee_user_name'] = isset($_POST['user_name'])?strtolower($_POST['user_name']):"";
...
class File
{
	...
	function __construct($name, $location)
	{
		...
		global $_CONFIG;
		$file = $this->location->getDir(true, false, false, 0).$this->getName();
		if (!file_exists($file)) $file = $_CONFIG['basedir'].$file;
		
		$this->type = File::getFileType($file);
		$this->size = File::getFileSize($file);
		$this->modTime = filemtime($file);
	}
